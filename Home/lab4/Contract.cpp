...
Contract::Contract(const char* name, double base_salary, int working_hours, double hourly_rate) : Employee(name, base_salary), working_hours(working_hours), hourly_rate(hourly_rate) {}
...
double Contract::get_salary() const{
    double temp = this->get_base_salary();
    return temp + hourly_rate * working_hours;
}
